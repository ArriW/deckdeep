...
def plot_monster_power_scaling():
    ...
    group_power_limits = [
        calculate_monster_group_power_limit(level) for level in levels
    ]
    ...
    fig.add_trace(
        go.Scatter(
            x=levels, y=baseline_powers, mode="lines", name="Baseline Monster Power"
        )
    )
    fig.add_trace(
        go.Scatter(
            x=levels,
            y=group_power_limits,
            mode="lines",
            name="Monster Group Power Limit",
        )
    )
