...
class CategoryColoredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if hasattr(record, 'category'):
            color = self.CATEGORY_COLORS.get(record.category.upper(), self.DEFAULT_COLOR)
        else:
...
class GameLogger(logging.Logger):
    ...
    def stop_timer(self, name, category='PERFORMANCE'):
        ...
        self.info(f"Timer '{name}' completed in {elapsed:.4f} seconds", category=category)
...
def setup_game_logger(name='game_logger', level=logging.DEBUG, log_file='game.log', max_lines=10000):
    logger = GameLogger(name, level)
    
    # Console Handler with colored output and simplified timestamp
    console_formatter = CategoryColoredFormatter('%(asctime)s - %(levelname)s - %(message)s', 
    ...
    file_formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s',
                                       datefmt='%Y-%m-%d %H:%M:%S')  # Show date and time, but no milliseconds
